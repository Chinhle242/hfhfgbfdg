...
function check_img($img,$t='') {
...

	{
		...
		if($t)	$img=str_replace('images/film',$web_link.'/images/thumbs/film',$img);

		else	$img= $web_link.'/'.$img;
...
function splitlink($name,$type, $class) {
...

	{
		...
		$fas= $fas.'<a href="'.$web_link.'/'.$type.'/'.replace(strtolower($dem[$i])).'.html" style="text-decoration:none;" class="'.$class.'">'.$dem[$i].'</a>, ';					}

	    $name = $fas.'<a href="'.$web_link.'/'.$type.'/'.replace(strtolower($dem[$d-1])).'.html" style="text-decoration:none;" class="'.$class.'">'.$dem[$d-1].'</a>';
...
//	$date= date('H  m s D d-m-Y',$date);
	...
	$kq = $dayofweek.', ngày '.$datemonth.', '.$time;
...
function replace_time($str)	{
	...
	if ($_COOKIE['LangCookie']	!= 'Vietnamese')		{
		...
		$str=str_replace('Phút','Phút',$str);

		$str=str_replace('phút','Phút',$str);		

		$str=str_replace('Tập','Tập',$str);	
...

{
...

	{
		...
		case 43: $name = 'Timvui'; break;
		
		case 44: $name = 'NCT'; break;
		
		case 40: $name = 'Movshare'; break;
		
		case 46: $name = 'Sendspace'; break;
		
		case 47: $name = '4share'; break;
...

{
	...
	if ($server == '1')
		...
		$relate_episode = '<div class="listserver" style="padding-left:15px" align="left">'.relate_episode($film_id,$episode_id,$film_name,$episode_lang).'</div>';
...

	{
...

		{
			...
			$relate_episode .= relate_episode($film_id,$episode_id,$film_name,$episode_lang,$s[$i]);
...

	{
		...
		{			
			...
			if($server_id)
				...
				$main = '<div class="listserver" style="padding-left:5px;padding-bottom:5px;" align="left"><b><font class="servername">+ Server '.server_list($server_id).':</font></b> '.$main.'</div>';
