...
$inp_arr = array(
		'name'	=> array(
			'table'	=>	'cat_name_Vietnamese',
			'name'	=>	'Tên thể loại',
			'type'	=>	'free'
		),
		'name_e'	=> array(
			'table'	=>	'cat_name_English',
			'name'	=>	'Cat Name ( English )',
			'type'	=>	'free'
		),	
		'name_cn'	=> array(
			'table'	=>	'cat_name_Chinese',
			'name'	=>	'Cat Name (Chinese)',
			'type'	=>	'free'
		),	
		'name_ascii'	=>	array(
...
##################################################
if ($mode == 'add') {
	if($level == 2)	acp_check_permission_mod('add_cat');
	if ($_POST['submit']) {
		$error_arr = array();
...
##################################################
if ($mode == 'edit') {	
	if($level == 2)	acp_check_permission_mod('edit_cat');
	if ($cat_id) {
		if (!$_POST['submit']) {
		...
		while ($cat = $mysql->fetch_array($cat_query)) {
			...
			echo "<tr><td class=fr_2 width=50%><a href='$link&cat_id=".$cat['cat_id']."'><b>".$cat['cat_name_Vietnamese']."</b></a> ( ".$tt." )</td><td class=fr_2 width='15%' align=center><a href='?act=film&mode=edit&cat_id=".$cat['cat_id']."'><b>Quản lý Phim</b></a></td><td class=fr_2 width='5%' align=center><a href=# onclick=check_del(".$cat['cat_id'].")><b>Xoá</b></a></td></tr>";
			echo "</table></td></tr>";
		}
...
##################################################
if ($mode == 'del') {
	if($level == 2)	acp_check_permission_mod('del_cat');
	if ($cat_id) {
		if ($_POST['submit']) {
