...
if ($mode == 'edit') {
	...
	if ($c_del_id) {
		...
		?><form method="post">WOULD YOU LIKE TO SCRUB?<br><input value="YES" name=submit type=submit></form><?
	...
	elseif ($comment_id) {
		...
		if ($tt) {
			...
			echo "<tr class=head_c>
					...
					<td class=title align=left>Có Tất Cả [ ".$tt." ] Commnent</td>
			...
			while ($r = $mysql->fetch_array($q)) {
			    ...
			    $content = admin_emotions_replace(text_tidy($r['comment_content']));
			...
			echo '<tr><td class=fr><input class=checkbox type=checkbox name=chkall id=chkall onclick=docheck(document.media_list.chkall.checked,0) value=checkall></td>
			     ...
			     <td colspan=3 align="center" class=fr>Với Comment Đã Chọn '.
				...
				'<input type="submit" name="do" value="Thực Hiện"></td></tr>';
