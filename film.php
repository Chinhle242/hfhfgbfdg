...
if ($list || $search || $quick_search || $category || $actor || $tag || $sactor || $sfilm || $sdirector ) 
...
elseif ($tag) 

	{
		...
		$t_film_tag = urldecode($tag);

		

		$order_sql = "WHERE film_tag LIKE '%".get_ascii($t_film_tag).", %' OR film_tag LIKE '%, ".get_ascii($t_film_tag)."%'  OR film_tag LIKE '".get_ascii($t_film_tag)."' AND film_show = 1  ORDER BY film_date DESC";

		

		$title = $keywords = $description = $lang[$_COOKIE['LANG']]['lang[tag]']." ". ucwords($t_film_tag);
...

	{
		...
		$web_keywords 	= 	$keywords.', '.$web_keywords;

		

		$web_des		=  	$description.', '.$web_des;
...

{
	...
	$trailers_button ='<a title="watch trailers" href="'.$web_link.'/xem-trailer/'.replace($r['film_name_ascii']).'_'.$trailers['trailers_id'].'.html" target="_blank"><img src="skin/vuap/images/watch_trailer.png" /></a>';
	...
	if (!$episode['episode_id'])		$watch = '';
		...
		$watch	= '<a href=" '.$web_link.'/xem-phim/online/'.replace($r['film_name_ascii']).'_'.$episode['episode_id'].'.html"><img src="http://vuaphim.vn/skin/vuap/images/watch_now.png" /></a></p>';

##$kiemtien	= '<a href="http://adf.ly/159484/'.$web_link.'/xem-phim/online/'.replace($r['film_name_ascii']).'_'.$episode['episode_id'].'.html"><img src="http://img252.imageshack.us/img252/2965/watchnowq8433256.png" /></a></p>';
	...
	$film_tag=splitlink(check_data($r['film_tag']),'tag','');	
...
$web_title = "Phim " .$title .' Online - '. check_data(ucwords($r['film_name_ascii'])); 
		

		$web_keywords 	= 	$title.', '.$web_keywords;

		
                $web_des =  cut_string(strip_tags($film_info),160);
