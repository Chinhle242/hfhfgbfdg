...
class HTMLForm {
	...
	function createForm($title,$inp_arr,$error_arr) {
		...
		foreach($inp_arr as $key=>$arr) {
			...
			elseif (ereg("^yes_no::*",$arr['type'])) {
				$ex_ar = explode('::',$arr['type']);
				$st= $ex_ar[0]($ex_ar[1]);
				$type = 'yes_no';
			}
			else $type = $arr['type'];
			echo "<tr><td class=fr width=20% style=\"padding-left:10px;\"><b>".$arr['name']."</b>".(($arr['desc'])?"<br>".$arr['desc']:'')."</td><td class=fr_2>";
			...
			switch ($type) {
				...
				case 'checkbox'	:	echo "<input value=1".(($arr['checked'])?' checked':'')." checked type=checkbox class=checkbox name=".$key.">"; break;
			}
			if ($error_arr[$key]) {
				echo ' ';
