...
$rss .= "<description>Music68.net | World Movie At Your Fingertips</description>\r\n";
$rss .= "<language>vi-vn</language>\r\n";
$rss .= "<copyright>Copyright (C) " . ucwords($title) . "</copyright>\r\n";
$rss .= "<ttl>60</ttl>\r\n";
$rss .= "<generator>Music68.net</generator> \r\n";

while ($row = mysql_fetch_array($results)){
	$m_time = date('D, d M Y H:i:s');
	$rss .= "\t<item>\r\n";
	$rss .= "\t\t<title>" . htmlspecialchars($row['film_name']) . " - " . check_data(htmlspecialchars($row['film_name_real'])) . "</title>\r\n";
	$rss .= "\t\t<description>
	<![CDATA[
	 <TABLE><TR><TD>
	<img src='".$row['film_img']."' width='200' height='270' alt='' /></TD>
    	<TD>
	<a href='".$homeURL."/thong-tin-info/".clean_feed(replace($row['film_name_ascii']))."/".clean_feed(replace($row['film_name_real']))."/f".$row['film_id'].".html' 
	title='" . htmlspecialchars($row['film_name']) . " - " . check_data(htmlspecialchars($row['film_name_real'])) ."' target='_blank'>
	<h1 /><b>Xem Phim " . htmlspecialchars($row['film_name']) . " - " . check_data(htmlspecialchars($row['film_name_real'])) ." Online</b></h1> </a>
	...
	<br />" .htmlspecialchars_decode($row['film_info'])."
	
	]]>
	</description>\r\n";
	
	//$rss .= "\t\t<link>".$homeURL."/?movie=/info/".$row['film_id']."/".clean_feed(replace($row['film_name_ascii']))."</link>\r\n";
	$rss .= "\t\t<link>".$homeURL."/thong-tin-info/".clean_feed(replace($row['film_name_ascii']))."/".clean_feed(replace($row['film_name_real']))."/f".$row['film_id'].".html</link>\r\n";
