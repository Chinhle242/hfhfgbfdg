...
if (preg_match('#veoh.com/browse/videos/category/([^/]+)/watch/([^/]+)#', $url, $id_sr)){
	...
	elseif (preg_match("#youtube.com/view_play_list([^/]+)#",$url,$id_sr)) {
		...
		$url=$web_link.'/'.'mediaplayer.swf?file=http://gdata.youtube.com/feeds/api/playlists/'.$id.'.xml&playlist=over&skin=$web_link/mediaplayer_skin.xml&autostart=tru&repeat=liste';
    }
	elseif (preg_match("#gdata.youtube.com/#",$url,$id_sr)) {
		$url=$web_link.'/'.'mediaplayer.swf?file='.$url.'&playlist=over&&autostart=true&skin=http://vuaphim.vn/stylish.swf&repeat=list';
    }
	...
	elseif (preg_match("#sevenload.com/videos/([^/-]+)-([^/]+)#",$url,$id_sr)) {
		...
		$url=$web_link.'/'.'player.swf?file='.$link."&skin=http://vuaphim.vn/video/skewd.xml&autostart=true";
	}
	elseif (preg_match('#2shared.com/([^/]+)#', $url, $id)){
	$url="http://www.2shared.com/flash/player.swf?plugins=http://vuaphim.vn/vuaphim.swf&plugin.thienduongviet=true&plugin.tdv_id=xml_video/$episode_id&autostart=true&skin=http://vuaphim.vn/skin.swf";	
	}
		...
		$url="http://www.novamov.com/player/player.swf?plugins=http://vuaphim.vn/vuaphim.swf&plugin.thienduongviet=true&plugin.tdv_id=movie/xml_video/$episode_id&skin=http://vuaphim.vn/video/skewd.xml";	
	}
        else if (preg_match("#movshare.net/video/(.*?)#s", $url)){
		$url="http://www.movshare.net/player/player.swf?plugins=http://vuaphim.vn/vuaphim.swf&plugin.thienduongviet=true&plugin.tdv_id=movie/xml_video/$episode_id&skin=http://vuaphim.vn/video/skewd.xml";	
	}
	 else if ((preg_match("#video.zing.vn/(.*?)#s", $url)) || (preg_match("#zippyshare.com/(.*?)#s", $url)) || (preg_match("#vidxden.com/(.*?)#s", $url))){
		$url="http://www.vidxden.com/player/player.swf?plugins=http://vuaphim.vn/phimnhanh360.swf&phimnhanh360.file=$url&phimnhanh360.img=&skin=http://vuaphim.vn/video/skewd.xml";	
	}
